...
public class FenixEduClientFactory {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FenixEduClientFactory.class);

    private static FenixEduClient INSTANCE;

    public static FenixEduClient getSingleton() throws FenixEduClientException {
        if (INSTANCE == null) {
            try {
                ...
                INSTANCE = new FenixEduClientImpl(config, new OAuthAuthorizationImpl(config));
            } catch (IOException e) {
